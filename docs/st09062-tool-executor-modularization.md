...
`packages/core/src/tools/executor.ts` was reduced from `356` lines to a `170` line public executor that stays below the `300` line planning cutoff while preserving the stable `./executor.js` import surface.

The extracted runtime modules are each focused on one responsibility and also remain below the planning cutoff:

- `packages/core/src/tools/executor-types.ts` (`68` lines)
