 ...
 */
export function createSearchConfluenceTool(request: ConfluenceRequest, logger: Logger) {
  return toolBuilder()
    .name('search-confluence')
    .description(
      'Search for pages in Confluence using keywords or CQL (Confluence Query Language). Returns matching pages with titles, IDs, and excerpts.'
    )
    ...
    .tag('knowledge-base')
    .usageNotes(
      "Use this to find relevant documentation, policies, or information in Confluence. You can search by keywords or use CQL for advanced queries (e.g., 'space=AI AND type=page'). Use get-confluence-page to retrieve full content of specific pages."
    )
    ...
    .schema(
      z.object({
        query: z
          .string()
          .describe(
            "Search query or CQL expression (e.g., 'payment processing' or 'space=BL3 AND title~payment')"
          ),
        limit: z
          .number()
          .optional()
    ...
    .implement(async ({ query, limit = 10 }) => {
      ...
      try {
        const response = await request.get('/wiki/rest/api/content/search', {
          headers: {
        ...
        if (results.length === 0) {
          logger.warn(
            'search-confluence returned NO RESULTS - this is a valid outcome, agent should not retry',
            {
              query,
              limit,
