...
For **GitHub**, complete both native relationship operations. Attach each child through the parent's `sub_issues` API, then add each blocking edge through the blocked issue's `dependencies/blocked_by` API. Both operations take the related issue's numeric REST `id`, not its issue number or GraphQL node ID. The `Parent` and `Blocked by` body sections are documentation only.

After publishing to a real tracker, read both relationship graphs back through the tracker API. Completion requires the parent to contain the exact published child set and every ticket's native blockers to match the approved graph. If either relationship type required a fallback, state that explicitly in the handoff.
