...
## Review Scope Note
- The lint failure is baseline repository debt and not introduced by ST-02006.
- Baseline errors are primarily:
  - `no-undef` in `packages/tools/examples/*`
  - `@typescript-eslint/no-empty-object-type` in shared `packages/tools/src/**/types.ts` files
- ST-02006 changed files were linted directly and pass.
- Baseline lint cleanup should be delivered in a separate out-of-scope maintenance story.
