 ...
 *   trustedVariables: {
 *     organizationName: 'Acme Corp',  // From config file
 *     confidential: true
 *   }
 * });
 *
 ...
 * const result = renderTemplate(template, {
 *   untrustedVariables: {
 *     datasetName: req.body.name,  // User input - will be sanitized
 *   }
 * });
 * ```
 */
 ...
 * loadPrompt('system', { organizationName: 'Acme' });
 *
 * // Explicit: separate trusted and untrusted
 * loadPrompt('system', {
 *   trustedVariables: { organizationName: 'Acme' },
 *   untrustedVariables: { datasetName: userInput }
 * });
 * ```
 */
